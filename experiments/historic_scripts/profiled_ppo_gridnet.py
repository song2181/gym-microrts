...
class CategoricalMasked(Categorical):
    def __init__(self, probs=None, logits=None, validate_args=None, masks=[], sw=None):
        ...
        if len(self.masks) == 0:
            ...
            with sw.timer("BoolTensor"):
                self.masks = masks.bool()
            with sw.timer("new logits"):
...
class Agent(nn.Module):
    ...
    def __init__(self):
        ...
        self.critic = nn.Sequential(
            nn.Flatten(), layer_init(nn.Linear(32 * 16 * 16, 256)), nn.ReLU(), layer_init(nn.Linear(256, 1), std=1)
        )

    def forward(self, x):
        return self.encoder(x)

    def get_action(self, x, action=None, invalid_action_masks=None, envs=None, sw=None):
        with sw.timer("encode->decode"):
            logits = self.actor(self.forward(x))
        with sw.timer("grid_logits"):
            grid_logits = logits.view(-1, envs.action_space.nvec[1:].sum())
        with sw.timer("split_logits"):
            split_logits = torch.split(grid_logits, envs.action_space.nvec[1:].tolist(), dim=1)

        if action is None:
            with sw.timer("getMasks"):
                invalid_action_masks = torch.tensor(np.array(envs.vec_client.getMasks(0))).to(device)
                invalid_action_masks = invalid_action_masks.view(-1, invalid_action_masks.shape[-1])
            with sw.timer("split_invalid_action_masks"):
            ...
            with sw.timer("multi_categoricals"):
                multi_categoricals = [
                    CategoricalMasked(logits=logits, masks=iam, sw=sw)
                    for (logits, iam) in zip(split_logits, split_invalid_action_masks)
                ]
            with sw.timer("sample"):
...
# summary(agent, next_obs.shape[1:])
    ...
    with sw.timer("main"):
            ...
            for step in range(0, args.num_steps):
                ...
                with torch.no_grad():
                    with sw.timer("inference"):
                        with sw.timer("values"):
                            values[step] = agent.get_value(obs[step]).flatten()
                        with sw.timer("action"):
                            action, logproba, _, invalid_action_masks[step] = agent.get_action(obs[step], envs=envs, sw=sw)
                ...
                with sw.timer("real_action"):
                    real_action = torch.cat(
                        [
                            torch.stack([torch.arange(0, mapsize, device=device) for i in range(envs.num_envs)]).unsqueeze(2),
                            action,
                        ],
                        2,
                    )
                    ...
                    for env_idx, valid_action_count in enumerate(valid_actions_counts):
                        ...
                        for c in range(valid_action_count):
                            ...
                            # print(env_idx, valid_actions[valid_action_idx])
                            valid_action_idx += 1
        ...
        with sw.timer("train"):
            ...
            for i_epoch_pi in range(args.update_epochs):
                ...
                # with sw.timer('load_state_dict'):
                #     target_agent.load_state_dict(agent.state_dict())
                for start in range(0, args.batch_size, args.minibatch_size):
                    end = start + args.minibatch_size
                    minibatch_ind = inds[start:end]
                    with torch.no_grad():
                        with sw.timer("idx_access"):
                            mb_advantages = b_advantages[minibatch_ind]
                        with sw.timer("idx_access2"):
                            idx_access = b_logprobs[minibatch_ind]
                        with sw.timer("idx_access3"):
                            idx_access2 = b_invalid_action_masks[minibatch_ind]
                    with sw.timer("norm_adv"):
                        if args.norm_adv:
                            mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)
                    with sw.timer("re-inference"):
                    ...
                    with sw.timer("pg_loss"):
                        with sw.timer("ratio"):
                            ratio = (newlogproba - idx_access).exp()
                        with sw.timer("approx_kl"):
                            with torch.no_grad():
                                approx_kl = (b_logprobs[minibatch_ind] - newlogproba).mean()

                        with sw.timer("actual pg_loss"):
                            # Policy loss
                    ...
                    with sw.timer("gradient"):
                        loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
                        optimizer.zero_grad()
                        with sw.timer("backward"):
                            loss.backward()
                        with sw.timer("clip_grad_norm_"):
